...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/comunidade_purchases?select=utm_source,utm_medium,utm_campaign,utm_content,utm_term,id&payment_status=eq.approved&created_at=gte.${since}&created_at=lt.${until}&limit=10000`,
      {
        headers: {
          apikey: KEY,
